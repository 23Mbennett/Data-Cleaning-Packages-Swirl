...
| the package authors, "lubridate has a consistent, memorable syntax, that makes working with dates fun instead
| of frustrating." If you've ever worked with dates in R, that statement probably has your attention.
...
| You are really on a roll!
  ...
  |==============                                                                                         |  14%
| There are three components to this date. In order, they are year, month, and day. We can extract any of these
| components using the year(), month(), or day() function, respectively. Try any of those on this_day now.
...
| You're the best!
  ...
  |==================                                                                                     |  17%
| Now try the same thing again, except this time add a second argument, label = TRUE, to display the *name* of
| the weekday (represented as an ordered factor).
...
| date-times. The now() function returns the date-time representing this exact moment in time. Give it a try and
| store the result in a variable called this_moment.

> this_moment <- now(this_day)
Warning message:
  In with_tz(Sys.time(), tzone) : Unrecognized time zone '2018-02-12'
...
| explicit. Repeat the same command, but add two dashes OR two forward slashes to "192012" so that it's clear we
| want January 2, 1920.
...
| That's a job well done!
  ...
  |==================================================                                                     |  48%
| lubridate is also capable of handling vectors of dates, which is particularly helpful when you need to parse
| an entire column of data. I've created a vector of dates called dt2. View its contents now.
...
> update(this_moment, hours = 8, minutes = 34, seconds = 55)
[1] "2018-02-13 03:34:55 EST"

| All that practice is paying off!

|=======================================================                                                |  53%
| It's important to recognize that the previous command does not alter this_moment unless we reassign the result
| to this_moment. To see this, print the contents of this_moment.
...
| Excellent work!
  ...
  |=========================================================                                              |  55%
| Unless you're a superhero, some time has passed since you first created this_moment. Use update() to make it
| match the current time, specifying at least hours and minutes. Assign the result to this_moment, so that
| this_moment will contain the new time.

> update(this_moment, hours = 20, minutes = 3)
[1] "2018-02-13 15:03:42 EST"

| Not exactly. Give it another go. Or, type info() for more options.

| Try update(this_moment, hours = 10, minutes = 16, seconds = 0), which will update the time to 10:16:00. Make
| sure to reassign the result to this_moment.

> this_moment <- update(this_moment, hours = 20, minutes = 3)
...
| tomorrow. You also know that your flight is scheduled to arrive in Hong Kong exactly 15 hours and 50 minutes
| after departure.
...
| departure. We will approach this by finding the current date in New York, adding 2 full days, then setting the
| time to 17:34.
...
| the time zone that we want: "America/New_York". Store the result in a variable called nyc. Check out ?now if
| you need help.
...
> nyc
[1] "2018-02-12 20:06:30 EST"

| Excellent work!
  
  |=====================================================================                                  |  67%
| Your flight is the day after tomorrow (in New York time), so we want to add two days to nyc. One nice aspect
| of lubridate is that it allows you to use arithmetic operators on dates and times. In this case, we'd like to
| add two days to nyc, so we can use the following expression: nyc + days(2). Give it a try, storing the result
...
> depart
[1] "2018-02-14 17:34:30 EST"

| You are quite good my friend!

|============================================================================                           |  74%
| Your friend wants to know what time she should pick you up from the airport in Hong Kong. Now that we have the
| exact date and time of your departure from New York, we can figure out the exact time of your arrival in Hong
| Kong.

...

|==============================================================================                         |  76%
| The first step is to add 15 hours and 50 minutes to your departure time. Recall that nyc + days(2) added two
| days to the current time in New York. Use the same approach to add 15 hours and 50 minutes to the date-time
...
|=====================================================================================                  |  83%
| Print the value of arrive to the console, so that you can tell your friend what time to pick you up from the
| airport.

> arrive
[1] "2018-02-15 22:24:30 HKT"
...
| Use mdy("June 17, 2008", tz = "Singapore") to parse the date with the correct time zone and store the result
| in last_time.
