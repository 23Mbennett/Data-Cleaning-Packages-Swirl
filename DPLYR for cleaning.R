...
|===                                                                                                    |   3%
| As you may know, "CRAN is a network of ftp and web servers around the world that store identical, up-to-date,
| versions of code and documentation for R" (http://cran.rstudio.com/). RStudio maintains one of these so-called
| 'CRAN mirrors' and they generously make their download logs publicly available
| (http://cran-logs.rstudio.com/). We'll be working with the log from July 8, 2014, which contains information
| on roughly 225,000 package downloads.

...

|=====                                                                                                  |   5%
| I've created a variable called path2csv, which contains the full file path to the dataset. Call read.csv()
| with two arguments, path2csv and stringsAsFactors = FALSE, and save the result in a new variable called mydf.
...
| Normally, this is something you would have to do on your own. Just to build the habit, type library(dplyr) now
...
| Instead of attempting to print the entire dataset, dplyr just shows us the first 10 rows of data and only as
| many columns as fit neatly in our console. At the bottom, we see the names and classes for any variables that
| didn't fit on our screen.
...
| You are quite good my friend!
  ...
  |=========================================                                                              |  40%
| Instead of specifying the columns we want to keep, we can also specify the columns we want to throw away. To
...
> slect(cran, -1:size)
Error in slect(cran, -1:size) : could not find function "slect"
> select(cran, -1:size)
...
| You can specify as many conditions as you want, separated by commas. For example filter(cran, r_version ==
                                                                                            | "3.1.1", country == "US") will return all rows of cran corresponding to downloads from users in the US running
| R version 3.1.1. Try it out.
...
| version that is less than or equal to "3.0.2". The up arrow on your keyboard may come in handy here. Don't
| forget your double quotes!

> filter(cran, Country == "IN", r_version <= "3.0.2")
Error in filter_impl(.data, quo) : 
Evaluation error: object 'Country' not found.
...
|==============================================================                                         |  60%
| Now, use filter() to fetch all rows for which size is strictly greater than (>) 100500 (no quotes, since size
| is numeric) AND r_os equals "linux-gnu". Hint: You are passing three arguments to filter(): the name of the
| dataset, the first condition, and the second condition.
...
|================================================================                                       |  62%
| Finally, we want to get only the rows for which the r_version is not missing. R represents missing values with
| NA and these missing values can be detected using the is.na() function.
...
> is.na(3, 5, NA, 10)
Error in is.na(3, 5, NA, 10) : 
4 arguments passed to 'is.na' which requires 1
> is.na(c(3, 5, NA, 10))
...
| 5, NA, 10)) will negate the previous command, thus telling us what is NOT NA.
...
Error: `r_version` (`r_version = !is.na()`) must not be named, do you need `==`?
> filter(cran, r_version == !is.na())
Error in filter_impl(.data, quo) : 
...
> 
  ...
  Error in Select(cran, size:ip_id) : could not find function "Select"
  > cran2 <- select(cran, size:ip_id)
  ...
  > desc(cran, ip_id)
  Error in desc(cran, ip_id) : unused argument (ip_id)
  > desc(cran2, ip_id)
  Error in desc(cran2, ip_id) : unused argument (ip_id)
  > arrange(crand2, desc(ip_id))
  Error in arrange(crand2, desc(ip_id)) : object 'crand2' not found
  ...
  | package, ip_id) will first arrange by package names (ascending alphabetically), then by ip_id. This means that
  | if there are multiple rows with the same value for package, they will be sorted by ip_id (ascending
  ...
  | mutate(cran3, correct_size = size + 1000) will create one new column that is equal to the original sizes plus
  | 1000.
  ...
  | Great job!
    ...
    |==================================================================================================     |  95%
  | That's not particularly interesting. summarize() is most useful when working with data that has been grouped
  ...
  | we'll look at how to take advantage of some other useful features of dplyr to make your life as a data analyst
  | much easier.
  ...
  Selection: 1
  What is your email address? milesabennett45@gmail.com